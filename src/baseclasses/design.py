...
class DesignID(SampleID):
    m_def = Section(
        a_eln=dict(hide=['sample_owner', 'certified_values', 'certification_institute'])
    )
...
class ActiveFactor(Factor):
    m_def = Section(
        ...
        a_eln=dict(
            properties=dict(
                order=['name', 'step', 'unit', 'minimum_value', 'maximum_value', 'mean']
            )
        ),
    )
...
class Design(Entity):
    ...
    normalized_factor_lower = Quantity(
        ...
        default=-1,
        description=('The lower value of normalized design.'),
    ...
    normalized_factor_upper = Quantity(
        type=np.dtype(np.int64),
        default=1,
        description=('The upper value of normalized design.'),
        a_eln=dict(component='NumberEditQuantity'),
    )

    active_factors = SubSection(section_def=ActiveFactor, repeats=True)

    passive_factors = SubSection(section_def=PassiveFactor, repeats=True)

    design_id = SubSection(section_def=DesignID)
    ...
    def normalize(self, archive, logger):
        ...
        if self.passive_factors is not None:
            start_passive = (
                len(self.active_factors) if self.active_factors is not None else 0
            )
