...
class HighLevel(ArchiveSection):
    binning = Quantity(
        type=np.dtype(np.float64), shape=[2], a_eln=dict(component='NumberEditQuantity')
    )

    ccd_read_area = Quantity(
        type=np.dtype(np.float64), shape=[4], a_eln=dict(component='NumberEditQuantity')
    )
    ...
    processing = Quantity(
        type=str,
        a_eln=dict(
            component='EnumEditQuantity', props=dict(suggestions=['Gain Normalized'])
        ),
    )
        ...
        if file_name.lower().endswith('.dm3'):
            ...
            try:
                ...
                high_level = HighLevel(
                    ...
                    number_of_frame_shutters=get_parameter(
                        [
                            'ImageList',
                            'TagGroup0',
                            'ImageTags',
                ...
                image_section = GatamScan(
                    ...
                    minimum_value=get_parameter(
                        [
                            ...
                            'Intensity',
                            'Range',
                            'Minimum Value (counts)',
                        ],
                        dm3_file.original_metadata,
                    ),
                    saturation_level=get_parameter(
                        [
                            'ImageList',
                            'TagGroup0',
                            'ImageTags',
