...
class SEMImage_Zeiss_Detector(Image):
    ...
    detector = Quantity(
        type=MEnum(['SE2', 'InLens']), a_eln=dict(component='EnumEditQuantity')
    )
    ...
    column_mode = Quantity(
        ...
        a_eln=dict(
            ...
            props=dict(suggestions=['Analytic', 'High Resoultion', 'Fish Eye']),
        ),
    )
    ...
    magnification = Quantity(type=str, a_eln=dict(component='StringEditQuantity'))
