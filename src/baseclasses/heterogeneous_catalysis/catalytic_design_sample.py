...
class DesignSampleID(SampleID):
    m_def = Section(a_eln=dict(hide=['sample_owner']))

    sample_short_name = Quantity(
        type=str, a_eln=dict(component='StringEditQuantity', label='Design id')
    )
...
class ActiveDesignParameter(ArchiveSection):
    ...
    def normalize(self, archive, logger):
        if self.factor is not None:
            self.name = (
                f'{self.factor.label}: Step {self.factor.step} - {self.factor.name}'
            )
...
class DesignSample(BasicSample):
    ...
    design = Quantity(
        type=Reference(Design), a_eln=dict(component='ReferenceEditQuantity')
    )
    ...
    active_parameters = SubSection(section_def=ActiveDesignParameter, repeats=True)

    passive_parameters = SubSection(section_def=PassiveDesignParameter, repeats=True)

    further_parameters = SubSection(section_def=FurtherDesignParameter, repeats=True)

    sample_id = SubSection(section_def=DesignSampleID)
    ...
    def normalize(self, archive, logger):
        ...
        if self.chemical_composition_or_formulas:
            ...
            formulas = [
                Atoms(Composition(formula.strip()).get_integer_formula_and_factor()[0])
