...
class MultiTargetSputteringObservables(ArchiveSection):
    ...
    temperature = Quantity(
        links=['http://purl.obolibrary.org/obo/PATO_0000146', 'http://purl.obolibrary.org/obo/XCO_0000058'],
        type=np.dtype(np.float64),
        unit=('°C'),
    )
...
class MultiTargetSputtering(LayerDeposition):
    ...
    targets = SubSection(
        section_def=TargetProperties,
        repeats=True)

    observables = SubSection(
        section_def=MultiTargetSputteringObservables,
        repeats=True)
