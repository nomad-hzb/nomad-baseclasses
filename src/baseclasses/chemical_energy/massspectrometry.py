...
class MassspectrometrySettings(ArchiveSection):
    ...
    ion_source = Quantity(type=str, a_eln=dict(component='StringEditQuantity'))
...
class MassspectrometrySpectrum(ArchiveSection):
    ...
    spectrum_data = Quantity(type=np.dtype(np.float64), unit='mbar', shape=['*'])
...
class Massspectrometry(BaseMeasurement):
    ...
    recipe = Quantity(type=str, a_eln=dict(component='StringEditQuantity'))

    time = Quantity(type=Datetime, shape=['*'])

    settings = SubSection(section_def=MassspectrometrySettings)

    data = SubSection(section_def=MassspectrometrySpectrum, repeats=True)
