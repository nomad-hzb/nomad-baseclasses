...
class AntiSolventQuenching(Quenching):
    m_def = Section(
        links=['https://purl.archive.org/tfsco/TFSCO_00001052'], label_quantity='name'
    )
    ...
    anti_solvent_2 = SubSection(section_def=PubChemPureSubstanceSectionCustom)
    ...
    anti_solvent_dropping_time = Quantity(
        links=[
            'https://purl.archive.org/tfsco/TFSCO_00002150',
            'https://purl.archive.org/tfsco/TFSCO_00002151',
        ],
        type=np.dtype(np.float64),
        unit=('s'),
        a_eln=dict(
    ...
    anti_solvent_dropping_flow_rate = Quantity(
        links=[
            'https://purl.archive.org/tfsco/TFSCO_00005091',
            'https://purl.archive.org/tfsco/TFSCO_00005094',
        ],
        type=np.dtype(np.float64),
        unit=('ul/s'),
        a_eln=dict(
            component='NumberEditQuantity',
            defaultDisplayUnit='ul/s',
            props=dict(minValue=0),
        ),
    )
    ...
    anti_solvent_dropping_height = Quantity(
        links=[
            'https://purl.archive.org/tfsco/TFSCO_00005092',
            'https://purl.archive.org/tfsco/TFSCO_00005093',
        ],
        type=np.dtype(np.float64),
        unit=('mm'),
        a_eln=dict(
...
class GasQuenching(Quenching):
    m_def = Section(links=['https://purl.archive.org/tfsco/TFSCO_00001077'])
    gas = Quantity(
...
class AirKnifeGasQuenching(GasQuenching):
    ...
    air_knife_pressure = Quantity(
        links=[
            'https://purl.archive.org/tfsco/TFSCO_00005021',
            'https://purl.archive.org/tfsco/TFSCO_00005027',
        ],
        type=np.dtype(np.float64),
        unit=('mbar'),
    ...
    air_knife_speed = Quantity(
        links=[
            'https://purl.archive.org/tfsco/TFSCO_00005025',
            'https://purl.archive.org/tfsco/TFSCO_00005026',
        ],
        type=np.dtype(np.float64),
        unit=('mm/s'),
    ...
    air_knife_angle = Quantity(
        links=[
            'https://purl.archive.org/tfsco/TFSCO_00005024',
    ...
    air_knife_distance_to_thin_film = Quantity(
        links=[
            'https://purl.archive.org/tfsco/TFSCO_00005023',
            'https://purl.archive.org/tfsco/TFSCO_00005028',
        ],
        type=np.dtype(np.float64),
