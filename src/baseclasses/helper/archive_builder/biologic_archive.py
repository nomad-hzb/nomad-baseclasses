...
def get_biologic_properties(metadata):
    ...
    battery_capacity_unit = ureg(metadata.get('battery_capacity_unit')) if metadata.get('battery_capacity_unit') is not None else ureg('Ah')
    properties.battery_capacity = metadata.get('battery_capacity') * battery_capacity_unit
...
def get_cv_properties(metadata):
    ...
    properties.initial_potential_measured_against = metadata.get('Ei_vs')       #   yadg gives numbers here. check with mps file if they somehow map to Ref and Eoc in mps file
...
def get_lsv_properties(metadata):
    ...
    properties.initial_potential = metadata.get('Ei')
    properties.initial_potential_measured_against = metadata.get('Ei_vs')       # 4=Ref?, 0=None?, 110,123???, 3 and 50=Eoc??
...
def get_eis_data(data, cycle):
    ...
    cycle.time = np.array(data.get('time')) * ureg('s') # TODO test if get instead of None stuff works TODO check unit s
...
def get_voltammetry_data(data, cycle):
    ...
    cycle.time = np.array(time.data) * ureg(time.attrs.get('units')) if time is not None else None
    cycle.current = np.array(current.data) * ureg(current.attrs.get('units')) if current is not None else None
    cycle.voltage = np.array(voltage.data) * ureg(voltage.attrs.get('units')) if voltage is not None else None
    cycle.charge = np.array(charge.data) * ureg(charge.attrs.get('units')) if charge is not None else None
...
def get_voltammetry_archive(data, metadata, entry_class, multiple=False):
    ...
    entry_class.datetime = datetime.fromtimestamp(data.get('time')[0]) # TODO check if really first value of time data is selcted like that
