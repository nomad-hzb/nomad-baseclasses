...
def get_mpp_hysprint_samples(entry_self, data):
    ...
    for sample_idx, sample in enumerate(data["samples"]):
        ...
        if entry_self.samples is not None and len(entry_self.samples) == len(data["samples"]):
            sample_entry = entry_self.samples[sample_idx]

        sample_entry.name = f"Sample {sample['id']} (in Box)"
        sample_entry.time = df.groupby(pd.Grouper(key="Timestamp", freq=f"{entry_self.averaging_grouping_minutes}Min"))[
            "Duration_h"].mean()
        sample_entry.temperature = df.groupby(pd.Grouper(
            key="Timestamp", freq=f"{entry_self.averaging_grouping_minutes}Min"))["InTemperatur"].mean()
        sample_entry.radiation = df.groupby(pd.Grouper(
            key="Timestamp", freq=f"{entry_self.averaging_grouping_minutes}Min"))["InEinstrahlung"].mean()
