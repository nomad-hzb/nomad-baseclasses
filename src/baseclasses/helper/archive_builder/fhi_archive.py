            ...
            with archive.m_context.raw_file(data_file) as f:
                ...
                while True:
                    if ' Data for range ' + str(datarange) in data:
                        ...
                        xrr_data_entry.angle = data[
                            ' Data for range ' + str(datarange)
                        ]['2THETA']
                        xrr_data_entry.intensity = data[
                            ' Data for range ' + str(datarange)
                        ]['Cnt2D1']
    ...
    for data_file in data_files:
        if os.path.splitext(data_file)[-1] == '.uxd':
            ...
            with archive.m_context.raw_file(data_file) as f:
                ...
                xrr_data_entry.angle_type = '2THETA'
                xrr_data_entry.angle = data[' Detector type  Scintillation counter'][
                    '2THETA'
                ]
                xrr_data_entry.intensity = data[
        ...
        if os.path.splitext(data_file)[-1] == '.ray':
            ...
            with archive.m_context.raw_file(data_file) as f:
                ...
                xrr_data_entry.apparent_roughness = data_nice['Apparent Roughness'][
                    'value'
                ]
                xrr_data_entry.chi_squared = data_nice['chi^2']['value']
                xrr_data_entry.diffuse_scattering = data_nice['Diffuse Scattering'][
                    'value'
                ]
