...
def calculatePVparametersFromJV(
    ...
    if (True in np.diff(np.signbit(j_interpolated[0]))) & (
        ...
        try:
            voc_ind = (
                np.where(
                    np.diff(
                        np.signbit(
                            j_interpolated[0])))[0].item(), np.where(
                    np.diff(
                        np.signbit(
                            j_interpolated[1])))[0].item())
            voc = (np.round(v_new[voc_ind[0]], digitsVOC),
            ...
            voc_ind = (
                np.where(
                    np.diff(
                        np.signbit(
                            j_interpolated[0])))[0][0], np.where(
                    np.diff(
                        np.signbit(
                            j_interpolated[1])))[0][0])
            voc = (np.round(v_new[voc_ind[0]], digitsVOC),
                   np.round(v_new[voc_ind[1]], digitsVOC))

        jsc_ind = (np.where(np.diff(np.signbit(v_new)))[
                   0].item(), np.where(np.diff(np.signbit(v_new)))[0].item())

        jsc = (np.round(j_interpolated[0][jsc_ind[0]], digitsJSC), np.round(
            j_interpolated[1][jsc_ind[1]], digitsJSC))

        if voc[0] > 0 and jsc[0] > 0:
            ff = (np.round((mpp[0][0] * mpp[0][1]) / (voc[0] * jsc[0]) * 100, digitsFF),
                  np.round((mpp[1][0] * mpp[1][1]) / (voc[1] * jsc[1]) * 100, digitsFF))
        else:
            ff = (0, 0)

        pce = (np.round(voc[0] * jsc[0] * ff[0] / 100, digitsPCE),
               np.round(voc[1] * jsc[1] * ff[1] / 100, digitsPCE))

        try:
            m1, b1 = np.polyfit(v_new[voc_ind[0]
                                      - lineFittingDataPoints:voc_ind[0]
                                      + lineFittingDataPoints], j_interpolated[0][voc_ind[0]
                                                                                  - lineFittingDataPoints:voc_ind[0]
                                                                                  + lineFittingDataPoints], 1)
            rs_backward = np.round(((-1 / m1) / cellArea) / 1E-3, digitsRS)
        except BaseException:
            rs_backward = np.nan

        try:
            m2, b2 = np.polyfit(v_new[voc_ind[1]
                                      - lineFittingDataPoints:voc_ind[1]
                                      + lineFittingDataPoints], j_interpolated[1][voc_ind[1]
                                                                                  - lineFittingDataPoints:voc_ind[1]
                                                                                  + lineFittingDataPoints], 1)
        ...
        try:
            m3, b3 = np.polyfit(v_new[jsc_ind[0]
                                      - lineFittingDataPoints
                                      * factor:jsc_ind[0]
                                      + lineFittingDataPoints
                                      * factor], j_interpolated[0][jsc_ind[0]
                                                                   - lineFittingDataPoints
                                                                   * factor:jsc_ind[0]
                                                                   + lineFittingDataPoints
                                                                   * factor], 1)
            i = 1
            while m3 > 0:
                m3, b3 = np.polyfit(v_new[jsc_ind[0] - lineFittingDataPoints
                                          + i:jsc_ind[0]
                                          + lineFittingDataPoints
                                          + i], j_interpolated[0][jsc_ind[0]
                                                                  - lineFittingDataPoints
                                                                  + i:jsc_ind[0]
                                                                  + lineFittingDataPoints
                                                                  + i], 1)
                i += 1
            rshunt_backward = np.round(
                ((-1 / m3) / cellArea) / 1E-3, digitsRSHUNT)

        except BaseException:
            rshunt_backward = np.nan

        try:
            m4, b4 = np.polyfit(v_new[jsc_ind[1] - lineFittingDataPoints
                                      * factor:jsc_ind[1]
                                      + lineFittingDataPoints
                                      * factor], j_interpolated[1][jsc_ind[1]
                                                                   - lineFittingDataPoints
                                                                   * factor:jsc_ind[1]
                                                                   + lineFittingDataPoints
                                                                   * factor], 1)
                                          ...
                                          * factor
                                          + i], j_interpolated[1][jsc_ind[1]
                                                                  ...
                                                                  * factor
                                                                  + i], 1)
                i += 1
            rshunt_forward = np.round(
                ((-1 / m4) / cellArea) / 1E-3, digitsRSHUNT)
    ...
    if enablePlot:
        ...
        plt.text(-0.2, -5, s, fontsize=14,
                 verticalalignment='top', bbox=props, alpha=0.3)
...
def get_jv_data(filedata):
    ...
    jv_dict['V_oc'] = list(
        np.abs([float(df.iloc[5, 1]), float(df.iloc[5, 2])]))
    ...
    _, _, _, _, RSHUNT, RS, mpp = calculatePVparametersFromJV(np.array(
        df_curves), "", printing=False, enablePlot=True, cellArea=jv_dict['active_area'], lineFittingDataPoints=20)
