...
def merge_sections(
    ...
    logger=None,  # : 'BoundLogger' = None,
) -> None:
    if update is None:
        return
    if section is None:
        section = update.m_copy(deep=True)
        return
    ...
    if not archive.m_context.raw_path_exists(file_name) or overwrite or merge:
        if merge and file_name.endswith('.json'):
            try:
                ...
                new_entry = entity.m_from_dict(file_json['data'])
                merge_sections(new_entry, entity)
                entity = new_entry
            except Exception:
                raise Exception
        ...
        archive.m_context.process_updated_raw_file(
            file_name, allow_modify=overwrite or merge
        )
        return True
    return False
