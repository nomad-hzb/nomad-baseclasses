...
class Etching(PreparationStep):
    m_def = Section(
        ...
        a_eln=dict(
            properties=dict(
                order=['preparation', 'etching_agent', 'washing', 'storage'],
            )
        ),
    )
...
class Delamination(PreparationStep):
    m_def = Section(
        a_eln=dict(
            properties=dict(
                order=['preparation', 'solute', 'solvent', 'washing', 'storage'],
            )
        )
    )
    ...
    solute = SubSection(
        ...
        section_def=SolutionChemicalNew,
        repeats=True,
    )
    solvent = SubSection(
...
class MXeneSolution(CompositeSystem):
    ...
    etching = SubSection(
        links=['http://purl.obolibrary.org/obo/CHMO_0001558'], section_def=Etching
    )
