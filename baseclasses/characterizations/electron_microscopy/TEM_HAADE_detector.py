        ...
        if file_name.lower().endswith(".tif"):
            ...
            try:
                ...
                illumination = Illumination(
                    magnification=get_parameter(["Acquisition_instrument", "SEM", "magnification"], tif_file.metadata),
                    illumination_index=get_parameter(["CZ_SEM", "ap_ill_index"], tif_file.original_metadata, 1),
                    spot_size=get_parameter(["CZ_SEM", "ap_spot_size"], tif_file.original_metadata, 1),
                    camera_length=get_parameter(["CZ_SEM", "ap_camera_length"], tif_file.original_metadata, 1),
                    scan_rotation=get_parameter(["CZ_SEM", "ap_scanrotation"], tif_file.original_metadata, 1)
                )

                microscope_configuration = MicroscopeConfiguration2(
                    x_value=get_parameter(["Acquisition_instrument", "SEM", "Stage", "x"], tif_file.metadata),
                    y_value=get_parameter(["Acquisition_instrument", "SEM", "Stage", "y"], tif_file.metadata),
                    z_value=get_parameter(["Acquisition_instrument", "SEM", "Stage", "z"], tif_file.metadata),
                    alpha_tilt=get_parameter(["CZ_SEM", "ap_stage_at_t"], tif_file.original_metadata, 1),
                    beta_tilt=get_parameter(["CZ_SEM", "ap_stage_at_m"], tif_file.original_metadata, 1)
                )

                image_section = HAADEScan(
                    file_name=os.path.basename(file_name),
                    image_length=get_parameter(["ImageLength"], tif_file.original_metadata),
                    image_width=get_parameter(["ImageWidth"], tif_file.original_metadata),
                    samples_per_pixel=get_parameter(["SamplesPerPixel"], tif_file.original_metadata),
                    pixel_size=get_parameter(["CZ_SEM", "ap_pixel_size"], tif_file.original_metadata, 1),
                    frame_time=get_parameter(["CZ_SEM", "ap_frame_time"], tif_file.original_metadata, 1),
                    noise_reduction=get_parameter(["CZ_SEM", "dp_noise_reduction"], tif_file.original_metadata, 1),
