...
def get_cv_properties(metadata):
    ...
    properties.limit_potential_1_measured_against = "Eoc" if metadata.get(
        "E1 (V) vs.") == "Eoc" else "Eref"
    properties.limit_potential_2 = metadata.get("E2 (V)")
    properties.limit_potential_2_measured_against = "Eoc" if metadata.get(
        "E2 (V) vs.") == "Eoc" else "Eref"
    properties.final_potential = metadata.get("Ef (V)")
    properties.final_potential_measured_against = "Eoc" if metadata.get(
        "Ef (V) vs.") == "Eoc" else "Eref"
